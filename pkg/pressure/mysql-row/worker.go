...
import (
	...
	"github.com/xuenqlve/common/relational_database/generate_sql"
...
func (w *Worker) tableDef(database, table string) (tableDef *mysql_schema.Table, err error) {
	...
	if tableDef, ok = def.(*mysql_schema.Table); !ok {
		err = fmt.Errorf("get mysql-row schema_store %s.%s is not *mysql_schema.Table", database, table)
		return
	}
	return tableDef, nil
}
...
func (w *Worker) generateSQL(row message.SQLRows, tableDef *mysql_schema.Table) (query string, args []any, err error) {
	...
	default:
		if row.Operation == schema_store.Insert.String() {
			return generate_sql.GenerateInsertSQL(row.Contents, tableDef)
		}
		if row.Operation == schema_store.Update.String() {
			return generate_sql.GenerateUpdateSQLByJoin(row.Contents, tableDef, true)
		}
	}
