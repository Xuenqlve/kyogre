...
func TestMySQLConfigMetadata(t *testing.T) {
	...
	t.Run("SchemaStore", func(t *testing.T) {
		...
		for _, key := range keys {
			...
			t.Logf("database:%s,table:%s columns:%v primary:%v uniqueIndex:%v", tableDef.Database, tableDef.Table, tableDef.Columns, tableDef.PrimaryIndex, tableDef.UniqueIndex)
		}
